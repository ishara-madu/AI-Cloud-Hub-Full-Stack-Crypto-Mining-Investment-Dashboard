...
const AdminDashboard = () => {
    ...
    const fetch = async () => {
      const [
        ...
        supabase
          .from("wallets")
          .select(
            "balance, total_deposited, total_withdrawn, total_commission",
          ),
        supabase
        ...
        supabase
          .from("user_packages")
          .select("package_id, price_paid, ai_packages(name)"),
        supabase
          .from("profiles")
          .select("id", { count: "exact", head: true })
          .eq("is_frozen", true),
      ...
      const totalDeposited = wallets.reduce(
        (s, w) => s + Number(w.total_deposited),
        0,
      );
      const totalWithdrawn = wallets.reduce(
        (s, w) => s + Number(w.total_withdrawn),
        0,
      );
      const totalCommission = wallets.reduce(
        (s, w) => s + Number(w.total_commission),
        0,
      );

      const todayStr = new Date().toISOString().split("T")[0];
      const todayTx = (txRes.data || []).filter((t: any) =>
        t.created_at.startsWith(todayStr),
      );
      const todayDeposits = todayTx
        .filter((t: any) => t.type === "deposit" && t.status === "approved")
        .reduce((s: number, t: any) => s + Number(t.amount), 0);
      const todayWithdrawals = todayTx
      ...
      setStats({
        ...
        todayDeposits,
        todayWithdrawals,
        todayNewUsers: (allProfilesRes.data || []).filter((p: any) =>
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const count = (allProfilesRes.data || []).filter((p: any) =>
  ...
  const generateAiInsights = async () => {
    ...
    try {
      ...
      setAiInsight(
        data?.insight || "Unable to generate insights at this time.",
      );
    } catch (err) {
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: `Rs ${stats.totalDeposited.toLocaleString()}`,
    ...
    },
    {
      ...
      value: `Rs ${stats.totalWithdrawn.toLocaleString()}`,
  ...
  const txTypeIcon = (type: string) => {
    if (type === "deposit")
      return <ArrowDownToLine className="w-3 h-3 text-blue-500" />;
    if (type === "withdrawal")
      return <ArrowUpFromLine className="w-3 h-3 text-red-500" />;
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="gradient-primary rounded-2xl p-6 text-primary-foreground shadow-lg">
        <p className="text-sm font-medium opacity-80 mb-3 flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="text-center">
            <p className="text-2xl font-heading font-bold">
              Rs {stats.todayDeposits.toLocaleString()}
            </p>
            <p className="text-xs opacity-80">Deposits</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-heading font-bold">
              Rs {stats.todayWithdrawals.toLocaleString()}
            </p>
          ...
          <Link to="/admin/deposits">
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl px-4 py-3 flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-yellow-600">
                {stats.pendingDepositsCount} pending deposits
              </span>
            </div>
          </Link>
        )}
          ...
          <Link to="/admin/withdrawals">
            <div className="bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3 flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-red-500">
                {stats.pendingWithdrawalsCount} pending withdrawals
              </span>
            </div>
          </Link>
        )}
      ...
      <Card className="shadow-neu border-0 bg-gradient-to-br from-card to-primary/5">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-heading flex items-center gap-2">
            <Brain className="w-4 h-4 text-primary" /> AI Analytics &
            Predictions
          </CardTitle>
        </CardHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="shadow-neu border-0">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-heading flex items-center gap-2">
              <BarChart3 className="w-4 h-4 text-primary" /> User Growth (7
              Days)
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card className="shadow-neu border-0">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              {
                label: "Total In (Deposits)",
                value: stats.totalDeposited,
                color: "bg-emerald-500",
                pct: 100,
              },
              {
                label: "Total Out (Withdrawals)",
                value: stats.totalWithdrawn,
                color: "bg-red-500",
                pct:
                  stats.totalDeposited > 0
                    ? (stats.totalWithdrawn / stats.totalDeposited) * 100
                    : 0,
              },
              {
                ...
                pct:
                  stats.totalDeposited > 0
                    ? (stats.totalCommission / stats.totalDeposited) * 100
                    : 0,
              },
              {
                label: "Net Retained",
                ...
                pct:
                  stats.totalDeposited > 0
                    ? (stats.totalBalance / stats.totalDeposited) * 100
                    : 0,
              },
            ].map((item) => (
            ...
            {pendingDeposits.map((d) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <Badge className="text-[9px] bg-yellow-500/20 text-yellow-600 border-yellow-500/30">
                    Pending
                  </Badge>
                </div>
              </div>
